...
import { X, CheckCircle, AlertCircle, AlertTriangle, Download } from 'lucide-react';
...
export const DishImportResultDialog: React.FC<DishImportResultDialogProps> = ({
  success,
  dishCount,
  errors,
  onClose,
            ...
            <div className="text-sm text-slate-600">
              {success ? (
                <>
                  ...
                  <div className="text-xs text-slate-400 mt-1">菜品库已完全替换</div>
                </>
              ) : (
            ...
            <div>
              ...
              <div className="bg-rose-50 rounded-lg p-4 space-y-2 max-h-60 overflow-y-auto">
                {errors.map((error, index) => (
                  <div key={index} className="text-sm text-rose-700">
                    {error}
                  </div>
                ))}
              </div>
