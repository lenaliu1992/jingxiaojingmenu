...
export const ImportResultDialog: React.FC<ImportResultDialogProps> = ({ result, onClose }) => {
  const hasErrors = result.errors.length > 0;
  const hasWarnings = result.warnings.length > 0;
  const successCount = result.meals.length;
                    ...
                    {(warning.mealName || warning.dishName) && (
                      <div className="text-xs text-amber-600 mt-1">
                        ...
                        {(warning.expected !== undefined || warning.actual !== undefined) && (
                          <> | 预期: ¥{warning.expected?.toFixed(2)} | 实际: ¥{warning.actual?.toFixed(2)}</>
                        )}
                      </div>
                    )}
                  </div>
